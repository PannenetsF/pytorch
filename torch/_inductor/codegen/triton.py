...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def index_to_str(
        ...
        if dtype in (torch.int32, torch.int64):
            index_dtype = cast(torch.dtype, dtype)
            if index_dtype != self.get_index_dtype_as_torch_dtype():
                return TritonIndexExprPrinter(triton_type(index_dtype)).doprint(renamed)
